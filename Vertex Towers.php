<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Vertex Towers</title>
    <link rel="stylesheet" href="Assets/Vertex Towers.css">
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600&display=swap" rel="stylesheet">

</head>
<body>
     <!-- php include 'loader.php'; ?> -->
   <!-- Navbar -->
    <?php include 'Navbar.php'; ?>
    <!-- Navbar ends  -->

    <!-- Hero Section Starts-->
    <section class="hero" role="region" aria-label="Intro">
  <div class="hero-wrapper">
    <div class="hero-left">
      <div class="hero-inner">
        <h1>VERTEX TOWERS</h1>
        <p class="lead">
          AT 
          <span class="rukshmani">Vertex Towers</span> boasts exceptional connectivity, putting everything you need right at your doorstep. It also offers spacious living, ideally located in the heart of the city.
        </p>
      </div>
    </div>
  </div>
</section>
    <!-- Hero Section Ends -->

<!-- Perspective section starts  -->
 <section class="perspectives">
  <div class="section-header">
    <h2>Perspectives <span>(05)</span></h2>
  </div>

  <div class="perspectives-grid">

    <div class="perspective-card">
      <img src="gallery/Vertex Towers-1.jpg" alt="Night View">
      <span class="label">NIGHT VIEW</span>
    </div>

    <div class="perspective-card">
      <img src="gallery/Vertex Towers-2.jpg" alt="Club View">
      <span class="label">CLUB VIEW</span>
    </div>

    <div class="perspective-card">
      <img src="gallery/Vertex Towers-3.jpg" alt="Plaza View">
      <span class="label">PLAZA VIEW</span>
    </div>

    <div class="perspective-card">
      <img src="gallery/Vertex Towers-4.jpg" alt="Side View">
      <span class="label">SIDE VIEW</span>
    </div>

  </div>
</section>
    <!-- Perspective section ends  -->

   <!-- product topology section starts -->
<section class="product-typology">
  <h2>Product Typology <span>(06)</span></h2>

  <div class="typology-grid">

    <div class="typology-item">
      <div class="icon-wrap">
        <img src="gallery/Aurum logo-1.jpg" alt="">
      </div>
      <h4>Building Structure</h4>
      <p>1B + G / H.P. + 14 Storey</p>
    </div>

    <div class="typology-item">
      <div class="icon-wrap">
        <img src="gallery/Aurum logo-2.jpg" alt="">
      </div>
      <h4>Number of Blocks</h4>
      <p>02 Nos.</p>
    </div>

    <div class="typology-item">
      <div class="icon-wrap">
        <img src="gallery/Aurum logo-3.jpg" alt="">
      </div>
      <h4>Retail Units</h4>
      <p>10 Nos.</p>
    </div>

    <div class="typology-item">
      <div class="icon-wrap">
        <img src="gallery/Aurum logo-4.jpg" alt="">
      </div>
      <h4>Retail RERA Area</h4>
      <p>282 Sq. Ft. Onwards</p>
    </div>

  </div>
  
</section>

<button id="scrollTopBtn" aria-label="Scroll to top">↑</button>
<?php include 'Footer.php'; ?>
<script src="Assets/Scroll button.js"></script>
<script src="Assets/Hameburger.js"></script>
<script src="Assets/Smother.js"></script>
</body>
</html>